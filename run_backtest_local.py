r"""
回测脚本 — 使用本地 Parquet 数据库（D:\Trae test\stock data\data\daily_qfq）。
分层抽样 100 只股票，缓存周线数据，复用缓存回测。
"""
...
CACHE_FILE = "backtest_cache_local_3000.pkl"
...
if all_signals:
    ...
    if bt.trades:
        ...
        trade_df.to_csv("trades_local.csv", index=False, encoding="utf-8-sig")
        print(f"\n交易明细已保存至 trades_local.csv ({len(bt.trades)} 笔)")
