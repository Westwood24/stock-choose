...
- **data_fetcher.py** — 数据获取层。腾讯源(`stock_zh_a_hist_tx`)，使用 `adjust="qfq"` 前复权数据，日线通过 `_daily_to_weekly()` resample 为周线。代码前缀映射：60/68开头用 `sh`，其余用 `sz`
...
- **backtest.py** — 两种退出模式：止损退出（`_find_stop_loss_exit`）和固定持仓周数。`MAX_POSITIONS` 限制同时持仓数。`run_backtest_with_cache()` 复用预加载的周线缓存，避免重复 API 调用
